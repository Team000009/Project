...
class Asset:
    ...
    def __init__(self):
        ...
        nasCom = nasUrl[3]['Company']
        nasSym = nasUrl[3]['Ticker']
        nasd = {}
        ...
        if index == "S&P 500":
            ...
            if comp in sp.keys():
                ...
                if comp == "Google" or comp == "Alphabet":
                    if comp=='Google':
                        comp='Alphabet (Class A)'
                    elif comp=='Alphabet':
                        comp = 'Alphabet (Class C)'
...
class Model:
    ...
    def makeModel (self):
        try:
            ...
            model = Sequential()
            model.add(LSTM(70, return_sequences=True, input_shape=(100, 1), activation='sigmoid'))
            model.add(LSTM(70, activation='sigmoid'))
            model.add(Dense(1))
            model.compile(loss='mean_squared_error', optimizer='adam')

            model.fit(X_train, y_train, validation_data=(X_test, ytest), epochs=5, batch_size=30, verbose=1,workers=4,
    ...
    def __init__(self, tickr, data,model):
        ...
        try:
            ...
            for i in range(self.dic[self.futInterval]):
                self.ls.append(pd.to_datetime(datetime.date.today()) + datetime.timedelta(days=i))
